...
export async function collectPage(options) {
  ...
  const classTokens = (el) => {
    const value = typeof el.className === "string" ? el.className : el.getAttribute("class");
    return value ? value.split(/\s+/).filter(Boolean) : [];
  };
  const isDenied = (el) => (el.id && isDeniedToken(el.id)) || classTokens(el).some(isDeniedToken);
  ...
  while (stack.length) {
    ...
    const record = {
      ...
      gap: isFlexOrGrid ? [cs.rowGap, cs.columnGap].map((v) => (v === "normal" ? null : px(v))) : null,
    ...
    if (tag === "img" && inNav) {
      const text = [el.id, el.getAttribute("class"), el.getAttribute("alt"), el.getAttribute("src")].join(" ").toLowerCase();
